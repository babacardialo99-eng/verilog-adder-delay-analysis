...
// ============================================================
// Generic Testbench Template (RCA / CBA / CBA/ any ILA-style adder)
// Assumptions:
//   - DUT ports: X, PS, Z, NS
...
module tb_cba_case #(parameter integer N = 4) (); // TODO: rename module (e.g., tb_rca_case, tb_cba_case)
...
always @(*) begin
  ...
  initial begin
    ...
    $display ("---------------------------");
    $display ("CBA sweep case: W=%0d bits",  W);// change <design> to your design name (e.g., RCA, CBA, CBA)
    ...
    if (RESULT !== expected) begin
      ...
      $fdisplay(tb_cba_sweep.csv_fd, "%0d,%0d", W, -1);
    end else begin
      ...
      $fdisplay(tb_cba_sweep.csv_fd, "%0d,%0.4f", W, settle_time); // change <design> to your design name (e.g., RCA, CBA, CBA)
    end
...
// ------ Top that instantiates all N cases ----------
  ...
  initial begin 
    ...
    $dumpfile ("tb_cba_sweep.vcd");
    $dumpvars (0, tb_cba_sweep); // change <design> to your design name (e.g., RCA, CBA, CBA)

    // Create CSV + header
    csv_fd = $fopen("CBA_delay_sweep.csv", "w");
    if (csv_fd == 0) begin
      $display("ERROR: could not open CBA_delay_sweep.csv for write");
      $finish;
    end
  ...
  // TODO 5:
  // Make 6 sweep points for CBA and CBA designs (N=1,2,4,16,32,64)
  ...
  // TODO 6: instantiate more cases for N=8,16,64,128
    ...
    tb_cba_case #(.N(16) ) uN16();

    // Test case for N = 32 stages.
    tb_cba_case #(.N(32) ) uN32();
    
    // Test case for N = 64 stages.
    tb_cba_case #(.N(64) ) uN64();
