...
module tb_rca_case #(parameter integer N = 4) (); // TODO: rename module (e.g., tb_rca_case, tb_cba_case)
  ...
  // -------------------------
  // Replace <design> with your DUT module name (RCA / CBA / CSA / etc.)
  // Ensure parameters match DUT parameter names.
   RCA #(.N(N), .m(m), .n(n), .k(k)) dut ( 
...
// ------------------------------------------------------------
  ...
  always @(*) begin
  // X = [B3 B2 B1 B0 A3 A2 A1 A0]
  //        ↑ upper     ↑ lower
    for (iu = 0; iu < N; iu = iu + 1) begin
      A[iu*m +: m]  = X[iu*n +: m];     // lower m bit of the stage
      B[iu*m +: m]  = X[iu*n + m +: m]; // upper m bit of the stage
  end
  end
  // ------------------------------------------------------------
  // Golden model
  ...
  initial begin
    ...
    $display("---------------------------------------------------------");
    $display("RCA sweep case: W=%0d bits", W);  // change <design> to your design name (e.g., RCA, CBA, CSA)
   ...
   for (iu = 0; iu < N; iu = iu + 1) begin
    if (iu == 0)
        X[iu*n +: n] = {4'b1111, 4'b1111}; //TODO:<fill the correct stimulus here>;   // {B0=1, A0=1}
    else
        X[iu*n +: n] = {4'b0000, 4'b1111}; //TODO:<fill the correct stimulus here>;   // {Bi=0, Ai=1}
    end
    PS = 1'b0;
    ...
    if (RESULT !== expected) begin
      ...
      $fdisplay(tb_RCA_sweep.csv_fd, "%0d,%0d", W, -1);
    end else begin
      ...
      $fdisplay(tb_RCA_sweep.csv_fd, "%0d,%0.4f", W, settle_time); // change <design> to your design name (e.g., RCA, CBA, CSA)
    end
...
module tb_RCA_sweep;
  ...
  initial begin
    ...
    $dumpfile("tb_RCA_sweep.vcd");
    $dumpvars(0, tb_RCA_sweep); // change <design> to your design name (e.g., RCA, CBA, CSA)

    // Create CSV + header
    csv_fd = $fopen("RCA_delay_sweep.csv", "w");
    if (csv_fd == 0) begin
  ...
  // Test case for N = 4 stage
  tb_rca_case #(.N(4))    uN4();

  // Test case for N = 8 stage
  tb_rca_case #(.N(8))    uN8();

  // Test case for N = 16 stage
  tb_rca_case #(.N(16))  uN16();

  // Test case for N = 64 stage
  tb_rca_case #(.N(64))   uN64();
